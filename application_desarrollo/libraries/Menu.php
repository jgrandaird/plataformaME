...
class Menu {
    ...
    public function construir_menu_modulo($menuModulo) {
        $indice = sizeof($this->arrayMenu());
        foreach ($menuModulo as $menu) {
            $this->construir_menu($indice, $menu["Etiqueta"], $menu["Funcion"], $menu["Imagen"], $menu["Identificador"]);
            $indice++;
        }
    }

    public function filtrar_menu($menuModulo) {

        $indice = 0;
        foreach ($this->arrayMenuGenerico as $menuGenerico) {
            foreach ($menuModulo as $menu) {
                if ($menuGenerico["Identificador"] == $menu) {
                    $this->construir_menu($indice, $menuGenerico["Etiqueta"], $menuGenerico["Funcion"], $menuGenerico["Imagen"], $menuGenerico["Identificador"]);
                    $indice++;
                }
            }
        }
